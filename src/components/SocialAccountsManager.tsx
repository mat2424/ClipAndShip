...
export const SocialAccountsManager = () => {
  ...
  return <div className="space-y-6">
      ...
      {/* Available Platforms */}
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 grid-cols-1 rounded-lg">
        {platforms.map(platformConfig => <SocialPlatformButton key={platformConfig.platform} platform={platformConfig.platform} name={platformConfig.name} color={platformConfig.color} icon={platformConfig.icon} isConnected={isConnected(platformConfig.platform)} isConnecting={isConnecting === platformConfig.platform} isLocked={platformConfig.locked} isPremiumRequired={platformConfig.tier === 'pro' && userTier === 'free' || platformConfig.tier === 'premium' && (userTier === 'free' || userTier === 'pro')} onConnect={() => handleConnect(platformConfig.platform)} />)}
      </div>

      {/* Connected Accounts */}
      {connectedAccounts.length > 0 && <div className="mt-8">
          ...
          return <ConnectedAccountCard key={account.id} account={account} platformName={platformConfig?.name || account.platform} onDisconnect={() => disconnectAccount(account.id)} />;
        })}
          </div>
        </div>}

      {/* Empty State */}
      {connectedAccounts.length === 0 && !loading && <div className="text-center py-8">
